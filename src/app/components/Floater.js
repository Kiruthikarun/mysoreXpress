...
import { Music2, X, Disc, Play, Radio, Disc3 } from 'lucide-react';
...
const Floater = () => {
  ...
  const platforms = [
    ...
    },
    {
      ...
      url: 'https://www.instagram.com/channel/AbaPE1u-fveWap0Q/'
    }
  ];
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (floaterRef.current && !floaterRef.current.contains(event.target) && isOpen) {
        setIsOpen(false);
      }
    };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-[#ff4a4a] rounded-full animate-ping opacity-60"></div>
      </button>
      ...
      <div className={`absolute bottom-full right-0 mb-4 transition-all duration-300 ${
        ...
        <div className="bg-white/95 backdrop-blur-md rounded-lg p-4 shadow-xl
          ...
          <div className="flex items-center justify-between mb-3">
            <span className="text-[#ff4a4a] font-synth text-sm">Listen & Follow</span>
            <Disc3 className="w-4 h-4 text-[#ff4a4a] animate-spin-slow" />
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-[#ff4a4a]/20">
            <div className="flex items-center space-x-3">
              <a 
                href="https://youtu.be/585SciTn3qU?feature=shared" 
                target="_blank" 
                rel="noopener noreferrer"
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  <p className="text-[#ff4a4a] font-synth text-xs truncate group-hover:underline">
                    Latest Music From Mysore Xpress
                  </p>
                </a>
              </div>
            </div>
